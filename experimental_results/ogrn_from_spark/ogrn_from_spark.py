...
conn = sqlite3.connect('ogrn_from_spark.db')
cursor = conn.cursor()
...
driver.get("https://spark-interfax.ru/search")
...
for name,street,site,tag in zip(df['name'].values,df['street'].values,df['site'].values,df['tag'].values):
    ...
    driver.get("https://spark-interfax.ru/search/?query="+str(name))
    time.sleep(2)
    ...
    for i_mad in html.split('class="footer"')[0].split('class="search-result-list__item"')[1:]:
        if street.lower() in i_mad.lower():
            n = n +1
            print(n,')',[re.findall(r'\d+', i_mad.split('ОГРН')[-1].split('>&middot')[0])[0]])
            
            print([street])
            #d.append(i_mad.split('ОГРН')[-1].split('class="company-card__block"')[0])
            cursor.execute('INSERT INTO my_table (site,tag,name,street,ogrn) VALUES ( ?, ?, ?, ?, ?)', (site,tag,name, street, re.findall(r'\d+', i_mad.split('ОГРН')[-1].split('>&middot')[0])[0]))
            conn.commit()
            break
#df['ogrn_mad'] = d
