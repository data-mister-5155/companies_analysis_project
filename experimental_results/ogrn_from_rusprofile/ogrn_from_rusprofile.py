...
conn = sqlite3.connect('ogrn_from_rusprofile.db')
cursor = conn.cursor()
...
driver.get("https://www.rusprofile.ru/search?query=%D0%A0%D0%A3%D0%A1%D0%A1%D0%9C%D0%90%D0%A0%D0%9A%D0%95%D0%A2&type=ul")
...
for name,street,site,tag in zip(df['name'].values,df['street'].values,df['site'].values,df['tag'].values):
    ...
    driver.get("https://www.rusprofile.ru/search?query="+str(name))
    time.sleep(2)
    ...
    for i_mad in html.split('class="footer-wrp"')[0].split('class="main-wrap__content"')[-1].split('class="list-element"'):
        if street.lower() in i_mad.lower():
            n = n +1
            print(n,')',[re.findall(r'\d+', i_mad.split('ОГРН:')[-1].split('</span>')[0])[0]])
            
            print([street])
            #d.append(i_mad.split('ОГРН')[-1].split('</span>')[0])
            cursor.execute('INSERT INTO my_table (site,tag,name,street,ogrn) VALUES ( ?, ?, ?, ?, ?)', (site,tag,name, street, re.findall(r'\d+', i_mad.split('ОГРН:')[-1].split('</span>')[0])[0]))
            conn.commit()
            break
#df['ogrn_mad'] = d
...
for name,street,site,tag in zip(df['gpt'].values,df['street'].values,df['site'].values,df['tag'].values):
    ...
    for i_mad in html.split('class="footer-wrp"')[0].split('class="main-wrap__content"')[-1].split('class="list-element"'):
        if street.lower() in i_mad.lower():
            n = n +1
            print(n,')',[re.findall(r'\d+', i_mad.split('ОГРН:')[-1].split('<br>')[0])[0]])
