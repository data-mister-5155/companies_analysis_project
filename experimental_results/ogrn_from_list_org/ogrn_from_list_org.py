...
conn = sqlite3.connect('ogrn_from_list_org.db')
cursor = conn.cursor()
...
for name,street,site,tag in zip(df['name'].values,df['street'].values,df['site'].values,df['tag'].values):
    ...
    driver.get("https://www.list-org.com/search?val="+str(name))
    time.sleep(2)
    ...
    for i_mad in html.split("</div></div><div class='mt-2'>")[0].split("<div class='card w-100 p-1 p-lg-3 mt-1'")[-1].split("<p><label><input class='form-check-input'"):
        if street.lower() in i_mad.lower():
            n = n +1
            print(n,')',[re.findall(r'\d+', i_mad.split('инн/кпп')[-1].split('class="company-card__block"')[0])[0]])
            
            print([street])
            #d.append(i_mad.split('ОГРН')[-1].split('class="company-card__block"')[0])
            cursor.execute('INSERT INTO my_table (site,tag,name,street,ogrn) VALUES ( ?, ?, ?, ?, ?)', (site,tag,name, street, re.findall(r'\d+', i_mad.split('инн/кпп')[-1].split('class="company-card__block"')[0])[0]))
            conn.commit()
            break
#df['ogrn_mad'] = d
...
for name,street,site,tag in zip(df['gpt'].values,df['street'].values,df['site'].values,df['tag'].values):
    ...
    for i_mad in html.split("</div></div><div class='mt-2'>")[0].split("<div class='card w-100 p-1 p-lg-3 mt-1'")[-1].split("<p><label><input class='form-check-input'"):
        if street.lower() in i_mad.lower():
            n = n +1
            print(n,')',[re.findall(r'\d+', i_mad.split('инн/кпп')[-1].split('<br>')[0])[0]])
